--Betrayer of the Archfiend
local s,id=GetID()
function s.initial_effect(c)
    Ritual.AddWholeLevelTribute(c,aux.FilterBoolFunction(Card.IsRace,RACE_FIEND))
...
function s.spcostfilter(c)
	return c:IsSetCard(0x45) and c:IsRitualMonster() and c:IsAbleToGraveAsCost()
end
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
    local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingMatchingCard(s.spcostfilter,tp,LOCATION_DECK,0,1,nil)
